...
exports.endWorkout = async (req, res) => {
  try {
    ...
    if (!session || session.endTime) {
      return res.status(404).json({ error: 'Workout session not found or already ended' });
    }
    ...
    if (!existingToday || existingToday._id.toString() === session._id.toString()) {
      ...
      if (user.milestoneProgress >= 30) {
        ...
        console.log(`🎉 Congrats ${user.name}! You've completed a 30-Day Fitness Challenge!`);
      }
...
exports.getUserWorkouts = async (req, res) => {
  try {
    ...
    const sessions = await WorkoutSession.find({ user: userId }).sort({ createdAt: -1 });
    res.json(sessions);
  } catch (error) {
